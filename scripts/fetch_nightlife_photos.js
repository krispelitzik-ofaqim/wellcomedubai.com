#!/usr/bin/env node
// Fetch real photos for each nightlife from Google Places API (New)
// Output: data/nightlife-photos.json -> { nightlifeId: { photos:[{name,attribution}], placeName } }
...
const OUT_FILE = path.join(__dirname, '..', 'data', 'nightlife-photos.json');
...
async function searchNightlife(name) {
  return postJson('https://places.googleapis.com/v1/places:searchText', {
    textQuery: `${name} Dubai nightlife`,
    maxResultCount: 1,
...
function parseNightlifes() {
  const text = fs.readFileSync(DATA_FILE, 'utf8');
  // Find the nightlife:[...] array boundaries
  const start = text.indexOf('nightlife:');
...
async function run() {
  const nightlifes = parseNightlifes();
  console.log(`Found ${nightlifes.length} nightlifes`);
  let existing = {};
  ...
  const out = { ...existing };
  let i = 0;
  for (const h of nightlifes) {
    i++;
    if (out[h.id] && out[h.id].photos && out[h.id].photos.length) {
      console.log(`[${i}/${nightlifes.length}] #${h.id} ${h.name} — already has ${out[h.id].photos.length} photos, skip`);
      continue;
    }
    try {
      const r = await searchNightlife(h.name);
      const place = (r.places || [])[0];
      if (!place) { console.log(`[${i}/${nightlifes.length}] #${h.id} ${h.name} — no place found`); continue; }
      ...
      console.log(`[${i}/${nightlifes.length}] #${h.id} ${h.name} -> ${photos.length} photos (${place.displayName?.text})`);
    } catch (e) {
      console.error(`[${i}/${nightlifes.length}] #${h.id} ${h.name} -> error: ${e.message}`);
    }
    await new Promise(r => setTimeout(r, 150)); // rate-limit gentle
  }
  fs.mkdirSync(path.dirname(OUT_FILE), { recursive: true });
  fs.writeFileSync(OUT_FILE, JSON.stringify(out, null, 2), 'utf8');
  console.log(`\n✓ Saved ${Object.keys(out).length} nightlife entries to ${OUT_FILE}`);
}
