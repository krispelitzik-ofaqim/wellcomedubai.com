#!/usr/bin/env node
// Fetch real photos for each kids from Google Places API (New)
// Output: data/kids-photos.json -> { kidsId: { photos:[{name,attribution}], placeName } }
...
const OUT_FILE = path.join(__dirname, '..', 'data', 'kids-photos.json');
...
function parseKidss() {
  ...
  // Find the kids:[...] array boundaries
  const start = text.indexOf('kids:');
...
async function run() {
  const kidss = parseKidss();
  console.log(`Found ${kidss.length} kidss`);
  let existing = {};
  ...
  const out = { ...existing };
  let i = 0;
  for (const h of kidss) {
    i++;
    if (out[h.id] && out[h.id].photos && out[h.id].photos.length) {
      console.log(`[${i}/${kidss.length}] #${h.id} ${h.name} — already has ${out[h.id].photos.length} photos, skip`);
      continue;
    }
    try {
      const r = await searchKids(h.name);
      const place = (r.places || [])[0];
      if (!place) { console.log(`[${i}/${kidss.length}] #${h.id} ${h.name} — no place found`); continue; }
      ...
      console.log(`[${i}/${kidss.length}] #${h.id} ${h.name} -> ${photos.length} photos (${place.displayName?.text})`);
    } catch (e) {
      console.error(`[${i}/${kidss.length}] #${h.id} ${h.name} -> error: ${e.message}`);
    }
    await new Promise(r => setTimeout(r, 150)); // rate-limit gentle
  }
  fs.mkdirSync(path.dirname(OUT_FILE), { recursive: true });
  fs.writeFileSync(OUT_FILE, JSON.stringify(out, null, 2), 'utf8');
  console.log(`\n✓ Saved ${Object.keys(out).length} kids entries to ${OUT_FILE}`);
}
