#!/usr/bin/env node
// Fetch real photos for each shopping from Google Places API (New)
// Output: data/shopping-photos.json -> { shoppingId: { photos:[{name,attribution}], placeName } }
...
const OUT_FILE = path.join(__dirname, '..', 'data', 'shopping-photos.json');
...
function parseShoppings() {
  const text = fs.readFileSync(DATA_FILE, 'utf8');
  // Find the shopping:[...] array boundaries
  const start = text.indexOf('shopping:');
...
async function run() {
  const shoppings = parseShoppings();
  console.log(`Found ${shoppings.length} shoppings`);
  let existing = {};
  ...
  const out = { ...existing };
  let i = 0;
  for (const h of shoppings) {
    i++;
    if (out[h.id] && out[h.id].photos && out[h.id].photos.length) {
      console.log(`[${i}/${shoppings.length}] #${h.id} ${h.name} — already has ${out[h.id].photos.length} photos, skip`);
      continue;
    }
    try {
      const r = await searchShopping(h.name);
      const place = (r.places || [])[0];
      if (!place) { console.log(`[${i}/${shoppings.length}] #${h.id} ${h.name} — no place found`); continue; }
      ...
      console.log(`[${i}/${shoppings.length}] #${h.id} ${h.name} -> ${photos.length} photos (${place.displayName?.text})`);
    } catch (e) {
      console.error(`[${i}/${shoppings.length}] #${h.id} ${h.name} -> error: ${e.message}`);
    }
    await new Promise(r => setTimeout(r, 150)); // rate-limit gentle
  }
  fs.mkdirSync(path.dirname(OUT_FILE), { recursive: true });
  fs.writeFileSync(OUT_FILE, JSON.stringify(out, null, 2), 'utf8');
  console.log(`\n✓ Saved ${Object.keys(out).length} shopping entries to ${OUT_FILE}`);
}
