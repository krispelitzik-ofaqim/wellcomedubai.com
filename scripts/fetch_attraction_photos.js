#!/usr/bin/env node
// Fetch real photos for each attraction from Google Places API (New)
// Output: data/attraction-photos.json -> { attractionId: { photos:[{name,attribution}], placeName } }
...
const OUT_FILE = path.join(__dirname, '..', 'data', 'attraction-photos.json');
...
async function searchAttraction(name) {
  return postJson('https://places.googleapis.com/v1/places:searchText', {
    textQuery: `${name} Dubai attraction`,
    maxResultCount: 1,
...
function parseAttractions() {
  ...
  // Find the attractions:[...] array boundaries
  const start = text.indexOf('attractions:');
...
async function run() {
  ...
  const out = { ...existing };
  let i = 0;
  for (const h of attractions) {
    i++;
    if (out[h.id] && out[h.id].photos && out[h.id].photos.length) {
      console.log(`[${i}/${attractions.length}] #${h.id} ${h.name} — already has ${out[h.id].photos.length} photos, skip`);
      continue;
    }
    try {
      const r = await searchAttraction(h.name);
      const place = (r.places || [])[0];
      if (!place) { console.log(`[${i}/${attractions.length}] #${h.id} ${h.name} — no place found`); continue; }
      ...
      console.log(`[${i}/${attractions.length}] #${h.id} ${h.name} -> ${photos.length} photos (${place.displayName?.text})`);
    } catch (e) {
      console.error(`[${i}/${attractions.length}] #${h.id} ${h.name} -> error: ${e.message}`);
    }
    await new Promise(r => setTimeout(r, 150)); // rate-limit gentle
  }
  fs.mkdirSync(path.dirname(OUT_FILE), { recursive: true });
  fs.writeFileSync(OUT_FILE, JSON.stringify(out, null, 2), 'utf8');
  console.log(`\n✓ Saved ${Object.keys(out).length} attraction entries to ${OUT_FILE}`);
}
