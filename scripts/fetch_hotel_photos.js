#!/usr/bin/env node
// Fetch real photos for each hotel from Google Places API (New)
// Output: data/hotel-photos.json -> { hotelId: { photos:[{name,attribution}], placeName } }
...
const OUT_FILE = path.join(__dirname, '..', 'data', 'hotel-photos.json');
...
function parseHotels() {
  ...
  // Find the hotels:[...] array boundaries
  const start = text.indexOf('hotels:');
...
async function run() {
  ...
  for (const h of hotels) {
    ...
    if (out[h.id] && out[h.id].photos && out[h.id].photos.length) {
      console.log(`[${i}/${hotels.length}] #${h.id} ${h.name} — already has ${out[h.id].photos.length} photos, skip`);
      continue;
    }
    try {
      const r = await searchHotel(h.name);
      const place = (r.places || [])[0];
      if (!place) { console.log(`[${i}/${hotels.length}] #${h.id} ${h.name} — no place found`); continue; }
      ...
      console.log(`[${i}/${hotels.length}] #${h.id} ${h.name} -> ${photos.length} photos (${place.displayName?.text})`);
    } catch (e) {
      console.error(`[${i}/${hotels.length}] #${h.id} ${h.name} -> error: ${e.message}`);
    }
    await new Promise(r => setTimeout(r, 150)); // rate-limit gentle
  }
  fs.mkdirSync(path.dirname(OUT_FILE), { recursive: true });
  fs.writeFileSync(OUT_FILE, JSON.stringify(out, null, 2), 'utf8');
  console.log(`\n✓ Saved ${Object.keys(out).length} hotel entries to ${OUT_FILE}`);
}
