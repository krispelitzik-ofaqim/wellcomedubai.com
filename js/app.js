          ...
          <div style="padding:10px;">
            ...
            <div style="display:flex;gap:6px;margin-top:6px;">
              ...
              ${category === 'hotels'
                ...
                : `<a href="https://www.google.com/search?q=${encodeURIComponent((item.nameEn || item.name) + ' Dubai' + (category === 'attractions' ? ' tickets opening hours' : ' menu hours'))}" target="_blank" onclick="event.stopPropagation()" style="flex:1;padding:5px;border-radius:4px;border:none;background:#E9C46A;color:#2C5F6E;font-size:0.65rem;text-align:center;text-decoration:none;font-family:Heebo;"><i class="fas fa-info-circle"></i> ${category === 'attractions' ? 'מחירים' : 'תפריט'}</a>`
              }
            </div>
          </div>
        </div>`;
...
function openDetail(category, id) {
  ...
  modal.innerHTML = `
    <div class="modal-sheet">
      ${(category === 'hotels' && window.HOTEL_PHOTOS && window.HOTEL_PHOTOS[item.id]?.photos?.length) || (category === 'attractions' && window.ATTRACTION_PHOTOS && window.ATTRACTION_PHOTOS[item.id]?.photos?.length) ? renderHotelPhotoSlider(item, (category === 'hotels' ? window.HOTEL_PHOTOS : window.ATTRACTION_PHOTOS)[item.id]) : `<div style="position:relative;">
