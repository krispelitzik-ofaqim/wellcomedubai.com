...
function initApp() {
  ...
  setupSearch();
  setupHashRouting();
  fetch('data/hotel-photos.json?v=4').then(r => r.ok ? r.json() : null).then(j => { if (j) { window.HOTEL_PHOTOS = j; if (currentPage === 'home') renderHome(); } }).catch(() => {});
...
async function ensureHotelPhotos() {
  ...
  try {
    const r = await fetch('data/hotel-photos.json?v=4');
